...
void	corridor(Game *game) {
	...
	for (int i = 0; i < 30 ; i += 1.0) {
		floor(textureIDs, game->get_MVP(), i, distance - rounded_distance);
	}

	for (int i = 0; i < 30 ; i += 1.0) {
		celling(textureIDs, game->get_MVP(), i, distance - rounded_distance);
	}

	for (int i = 0; i < 30; i++) {
		wall(textureIDs, game->get_MVP(), game->get_map()[i], i, distance - (int)distance);
	}

	display_player(textureIDs, game->get_MVP(), game->get_pos(), game->get_height());
}
